...
Theorem mult_n_1 : forall n m: nat,
	m * (1 + n) = m + m * n.
Proof.
...
Proof.
	...
	-simpl. rewrite mult_n_1. rewrite IHn. reflexivity. 
	Qed.
...
Theorem all3_spec : forall b c : bool,
  ...
  = true.
Proof.
	intros. destruct b, c; simpl; reflexivity.
Qed.
...
Theorem bin_to_nat_pres_incr : 
	forall (m:bin), S (bin_to_nat m) = bin_to_nat (incr m).
Proof.
	intros.
	assert (H : forall (n:nat), S n = n + 1).
	  { intros.
	    induction n; simpl; try reflexivity.
	    rewrite IHn. reflexivity.
	  }
	...
	- simpl. rewrite add_0_r.
	  rewrite H. reflexivity.
  - simpl. do 2 rewrite add_0_r.
  		rewrite <- IHm. simpl. rewrite (H (bin_to_nat m)). 
  		rewrite (H (bin_to_nat m + (bin_to_nat m + 1))). rewrite H.
  		rewrite add_assoc. reflexivity. 
Qed.
...
Theorem bin_nat_bin : forall n, nat_to_bin (bin_to_nat n) = n.
Proof.
...
Definition is_zero (b : bin) : bool :=
	match b with
	| Z => true
	| _ => false
	end.

Fixpoint normalization (b : bin) : bin :=
	match b with
	| Z => Z
	| B0 x => if (is_zero(normalization x)) then Z
						else B0 x
	| B1 x => B1 (normalization x)
	end.
	
	
Compute B0 (B1 Z).
Compute normalization (B0 (B1 Z)).
Compute B1 (B0 Z).
Compute normalization (B1 (B0 Z)).
Compute B1 (B0 (B0 (B0 Z))).
Compute normalization (B1 (B0 (B0 (B0 Z)))).
Compute B0 Z.
Compute normalization (B0 Z).
Compute B0 (B0 (B0 Z)).
Compute normalization (B0 (B0 (B0 Z))).
