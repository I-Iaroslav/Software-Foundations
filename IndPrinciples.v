...
Definition rgb_ind' := forall P : rgb -> Prop,        
       P red ->
       P green ->
       P blue ->
       forall r : rgb, P r.
...
Definition add_comm'''_r (n m:nat) : Prop :=
  n + m = m + n.
...
Proof.
	intros.
  apply nat_ind; unfold add_comm'''_r.
  - rewrite -> add_0_r. reflexivity.
  - intros. simpl. rewrite <- H.
    rewrite <- plus_n_Sm. reflexivity.  
Qed.
    
Definition add_assoc'''_r (n m p:nat) : Prop :=
  n + (m + p) = (n + m) + p.

Definition add_assoc'''_r' : nat -> Prop :=
...
Theorem add_assoc''' : forall n : nat,
  add_assoc'''_r' n.
Proof.
  apply nat_ind; unfold add_assoc'''_r'; simpl.
  - reflexivity.
  - intros.
...
Print nat_ind.
Definition better_t_tree_ind_type := forall (X : Type)
      (P : t_tree X -> Prop), 
       P t_leaf -> 
      (forall b1 x b2, P b1 -> P b2 -> P (t_branch (b1, x, b2))) ->
       forall t : t_tree X, P t
.

Inductive t_tree' (X : Type) : Type :=
| t_leaf'
| t_branch' (b1 b2 : t_tree' X) (x : X) : t_tree' X.
...
Definition better_t_tree_ind : better_t_tree_ind_type
  := fun (X : Type) (P : t_tree X -> Prop) (f : P t_leaf)
         (f0 : forall b1 x b2, P b1 -> P b2 -> P (t_branch (b1, x, b2)))
       => fix F t : P t
            := match t with
               | t_leaf => f
               | t_branch (b1, x, b2) => f0 b1 x b2 (F b1) (F b2)
               end.
