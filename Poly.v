...
Compute (combine ([3;5;6])([true; false; true; false])).
...
Compute split [(1,false);(2,false);(3, true);(4, true)].
...
Definition filter_even_gt7 (l : list nat) : list nat :=
	filter even (filter (fun n => negb(n <=? 7)) l).
...
Compute  fold (fun (b:bool) (n:nat) => if b then n + 1 else n + 10) 
...
Definition fold_map {X Y : Type} (g : X -> Y) (l : list X) : list Y
  := fold (fun x y => cons (g x) y) l nil.

Compute (fold_map (fun x => 3 * x) ([1;2;3])).
