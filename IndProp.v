...
Proof.
	intros. 
	apply ev_Even_iff. apply ev_Even_iff in H. apply ev_Even_iff in H0.
	unfold Even in *. inversion H0. inversion H.
	exists (x + x0). rewrite H1. rewrite H2.
	do 3 rewrite double_plus. rewrite add_assoc. rewrite add_assoc.
	rewrite add_comm. rewrite add_assoc. rewrite add_assoc.
	rewrite (add_comm (x + x0)). rewrite add_assoc.
	rewrite add_comm. rewrite add_assoc. rewrite add_assoc.
	reflexivity. 
Qed.
(** [] *)
...
Proof.
	...
	apply (ev_ev__ev (n+n)).
	- apply ev_Even_iff. apply ev_Even_iff in H. apply ev_Even_iff in H0.
		unfold Even in *. destruct H as [a Ha]. destruct H0 as [b Hb].
		rewrite double_plus in Ha. rewrite double_plus in Hb.
		exists (a + b). rewrite double_plus. 
		rewrite add_comm.
		rewrite add_assoc. rewrite add_assoc. rewrite add_comm. 
		...
		rewrite add_assoc. rewrite add_assoc. rewrite add_comm.
		rewrite add_assoc. rewrite add_assoc. 
		rewrite (add_comm m n). rewrite Ha. 
		rewrite add_comm. rewrite add_assoc. rewrite add_assoc.
		rewrite (add_comm b a). reflexivity.
...
Proof.
	...
	- right. apply O_le_n.
	- specialize IHn with m. destruct IHn.
...
Theorem add_le_cases_help: forall n p, 
	n <= p -> p <= n -> n = p.
Proof. 
	induction n, p; intros.
	- reflexivity.
	- inversion H0.
	- inversion H.
	- apply Sn_le_Sm__n_le_m in H. apply Sn_le_Sm__n_le_m in H0.
		rewrite (IHn p H H0). reflexivity.
Qed.

Theorem add_le_cases_help_2: forall n p q, 
	p + q <= p + n -> q <= n. 
Proof. 
	intros. 
	induction p; simpl in *.
	- apply H.
	- apply Sn_le_Sm__n_le_m in H. apply IHp in H.
		apply H.
Qed.
...
Proof.
	...
	- left. apply O_le_n.
	- assert (n + m <= p + q).
	  { apply le_trans with (S n + m); try assumption.
	    simpl. repeat constructor.
	  }
	  apply IHn in H0 as H1. destruct H1; try (right; assumption).
	  destruct (lt_ge_cases n p). left; assumption. right. 
	  	unfold lt, ge in *.
	  assert (np : n = p). apply (add_le_cases_help n p H1 H2). subst.
		apply add_le_cases_help_2 in H0. apply H0.
Qed.
...
Proof.
	intros.
	induction p.
	- simpl. rewrite add_comm. rewrite (add_comm m 0). simpl. apply H.
	- simpl. rewrite add_comm. rewrite (add_comm m (S p)). simpl.
		apply n_le_m__Sn_le_Sm.
		rewrite add_comm. rewrite (add_comm p m). apply IHp.
Qed.
...
Proof.
	...
	- rewrite add_comm. simpl. apply le_S. rewrite add_comm. apply IHp.
Qed.
...
Theorem n_lt_m__n_le_m : forall n m,
  ...
  n <= m.
Proof.
	intros. apply le_S in H. apply Sn_le_Sm__n_le_m. apply H.
Qed.
...
Theorem plus_lt : forall n1 n2 m,
  ...
  n1 < m /\ n2 < m.
Proof.
	intros. split; unfold lt, ge in *.
	- 	induction n2.
		+ rewrite add_comm in H. simpl in *. apply H.
		+ apply le_S in H. apply Sn_le_Sm__n_le_m in H. apply IHn2.
			rewrite add_comm in H. simpl in *. rewrite add_comm in H. apply H.
	- induction n1.
		+ simpl in *. apply H.
		+ apply le_S in H. apply Sn_le_Sm__n_le_m in H. apply IHn1.
			simpl in *. apply H.
Qed.
...
Proof.
	...
	- apply O_le_n.
	- discriminate H.
	- apply n_le_m__Sn_le_Sm. apply IHn. inversion H. 
		reflexivity.
Qed.
...
Theorem R_equiv_fR_help : forall m, R m 0 m.
Proof.
	intros.
	induction m.
	- apply c1.
...
Proof.
	...
	- unfold fR. intros.
		...
		+ simpl in *. inversion IHR. rewrite add_comm in H1. 
			inversion H1. rewrite add_comm. reflexivity.
		+ rewrite add_comm. apply IHR.
	- unfold fR. intros.
		rewrite <- H. generalize dependent m. induction n; intros.
		+ simpl in *. rewrite add_comm. rewrite add_comm in H.
		  simpl in *. apply R_equiv_fR_help.
		+ rewrite add_comm. simpl in *. apply c3. rewrite add_comm.
			specialize IHn with (S m). apply c4. apply c3.
		  simpl in *. apply IHn. rewrite add_comm in H. simpl in H.
...
Inductive subseq : list nat -> list nat -> Prop :=
	| ss1 : subseq [] []
	| ss2 x l1 l2 : subseq l1 l2 -> subseq  l1 (x :: l2)
	| ss3 x l1 l2 : subseq l1 l2 -> subseq  (x :: l1) (x :: l2)
.
...
Proof.
	intros.
	induction H; simpl.
	{ induction l3. apply ss1. apply ss2. apply IHl3. }
	- apply ss2. trivial.
	- apply ss3. trivial.
Qed.
...
Proof.
	intros.
	split; intros; generalize dependent s1.
	- induction s2; simpl in *; intros; inversion H.
		+ reflexivity.
		+ subst. inversion H3. subst. simpl.
			apply IHs2 in H4. rewrite H4. reflexivity.
	- induction s2; simpl in *; intros; inversion H.
		+ constructor.		
		+ subst.
			replace (x :: s2) with ([x] ++ s2); trivial.
			repeat constructor. apply IHs2. reflexivity.
Qed.
(** [] *)
...
Proof.
	...
	- intros. destruct H.
		...
		+ rewrite IHexp_match. simpl. reflexivity.
		+ rewrite IHexp_match. destruct (re_not_empty re1); reflexivity.
	- intros. induction re; simpl in *.
		+ discriminate H.
		+ exists []. constructor.
		+ exists [t]. constructor.
		+ destruct re_not_empty; simpl in *.
			* destruct (IHre2 H). destruct (IHre1 eq_refl). 
				exists (x0 ++ x). constructor. apply H1. apply H0.
			* discriminate H.
		+ do 2 destruct re_not_empty; simpl in *.
			* destruct (IHre1 H). 
		...
		+ destruct re_not_empty; simpl in *.
			* destruct (IHre H).
				exists (x). rewrite <- (app_nil_r T x).
				apply MStarApp. assumption. constructor.
			* exists []. constructor.
Qed.
(** [] *)
...
Proof.
  ...
  - simpl in *. intros. rewrite app_length in H.
		...
		+ apply IH1 in H. do 4 destruct H. destruct H0. subst.
			...
			* repeat rewrite app_assoc. 
				reflexivity.
			* split. apply H0. intros. repeat rewrite app_assoc. 
    			constructor. rewrite <- app_assoc. apply H1. assumption.
    + apply IH2 in H. do 4 destruct H. destruct H0. subst.
  ...
  - simpl. intros. apply plus_le in H. destruct H.
  		apply IH in H. do 4 destruct H. destruct H1. subst.
		exists x, x0, x1. split. 
		* repeat rewrite app_assoc. 
			reflexivity.
		* split. assumption. intros. repeat rewrite app_assoc.
    		repeat rewrite <- app_assoc. 
    		constructor. apply H2. 
  - simpl. intros. apply plus_le in H. destruct H.
  		apply IH in H0. do 4 destruct H0. destruct H1. subst.
		exists x, x0, x1. split.
		* repeat rewrite app_assoc. 
			reflexivity.
		* split. assumption. intros. repeat rewrite app_assoc.
  ...
  - simpl. intros. inversion H. apply pumping_constant_0_false in H1.
  	...
  	- simpl in *. intros. rewrite app_length in H. 
  		destruct s1.
  		+ simpl in *. apply IH2 in H. do 3 destruct H.
  			exists x, x0, x1. apply H.
  		+ exists [], (x :: s1), s2. split.
  			* simpl in *.
  				reflexivity.
  			* split. discriminate. intros. simpl.  
    	...
    	+ destruct (lt_ge_cases (pumping_constant re1) (length s1)).
    		* apply n_lt_m__n_le_m in H0. apply IH1 in H0. do 4 destruct H0. 
    			...
    			-- apply (le_plus_trans (length x + length x0) (pumping_constant re1) (pumping_constant re2)) in H2.
    				 assumption.
    			-- intros. specialize H3 with m. repeat rewrite app_assoc.
    				 apply MApp. rewrite <- app_assoc. assumption.
    				 assumption.
    		* apply IH2 in H. do 4 destruct H. destruct H1. destruct H2. subst.
				exists (s1 ++ x), x0, x1. repeat split; trivial.
				...
				-- rewrite app_length. unfold ge in H0.
					...
					repeat rewrite <- add_assoc. apply H; trivial.
				-- intros. repeat rewrite <- app_assoc. 
					 specialize H3 with m. apply MApp; assumption.
  - simpl. intros. apply plus_le in H. destruct H.
		...
		destruct (lt_ge_cases (pumping_constant re) (length s1)).		
  		+ apply n_lt_m__n_le_m in H0. apply IH1 in H0. do 4 destruct H0.
  			...
  			* intros. repeat rewrite app_assoc. apply MStarApp.
  				repeat rewrite <- app_assoc. 
  				specialize H3 with m. assumption. assumption.
  		+ destruct s1.
  			* simpl in *. apply IH2 in H. do 4 destruct H. destruct H1.
  				destruct H2. subst. exists x, x0, x1. repeat split; trivial.
  			* exists [], (x :: s1), s2. unfold ge in H0.  
  				repeat split; simpl in *; trivial.
  				-- intros H1. inversion H1. 
  				-- intros. apply napp_star. assumption. assumption.
Qed.
...
Proof.
	...
	- split.
		+ intros. inversion H. apply H1 in H0. destruct H0.
...
Proof.
	...
	- unfold not. intros. destruct H0.
		+ subst. inversion H. destruct (eqbP n n) in H.
			* discriminate H.
			* unfold not in H0. apply H0. reflexivity.
		+ destruct (eqbP n x) in H.	
			* subst. discriminate H.
			* simpl in *. apply IHl in H. unfold not in H.
...
Inductive nostutter {X:Type} : list X -> Prop :=
	 | nst_e 		: nostutter []
	 |	 nst_1 x 	: nostutter [x]
	 | nst_increase x1 x2 l 
	 	 	(H1 : nostutter (x1 :: l)) (H2 : x1 <> x2) 
	 	 					: nostutter (x2 :: (x1 :: l))
.
...
Inductive iom {X:Type}
  : list X -> list X -> list X -> Prop
  := | iom0 : iom [] [] []
	   | iom1 l1 l2 l x : iom l1 l2 l -> iom (x :: l1) l2 (x :: l)
	   | iom2 l1 l2 l x : iom l1 l2 l -> iom l1 (x :: l2) (x :: l)
	.


Theorem iom_test : forall (X : Set) (test : X->bool) (l l1 l2 : list X),
	iom l1 l2 l ->
	All (fun n => test n = true) l1 ->
	All (fun n => test n = false) l2 ->
	filter test l = l1.
Proof.
	intros * H. induction H; intros; simpl in *.
	- reflexivity.
	- destruct test.
		+ inversion H0. rewrite (IHiom H3 H1). reflexivity.
		+ inversion H0. discriminate H2.
	- destruct test.
		+ inversion H1. discriminate H2.
		+ inversion H1. apply (IHiom H0 H3).
Qed.
...
Inductive sublist {X:Type}
  : list X -> list X -> Prop
  := | sub_e l : sublist [] l
	   | sub_1 l1 l x : sublist l1 l -> sublist (x :: l1) (x :: l)
	   | sub_2 l1 l x : sublist l1 l -> sublist l1 (x :: l)
	.
	
Theorem sub_test : forall (X : Set) (test : X->bool) (l l1 : list X),
	sublist l1 l ->
	All (fun n => test n = true) l1 ->
...
Proof.
	intros * H. induction H; intros; simpl in *.
	- apply O_le_n.
	- destruct H0. destruct (test x).
		+ simpl in *. apply le_n_S. apply IHsublist.
			assumption.
		+ discriminate H0.
	- destruct test.
		+ simpl in *. apply le_S in IHsublist. apply IHsublist.
			assumption.
...
Theorem pal_rev {X : Type} : forall (l : list X), 
	pal l -> l = rev l.
Proof.
	intros. induction H; simpl in *.
	- reflexivity.
	- reflexivity.
	- rewrite rev_app_distr. rewrite <- IHpal. 
		simpl. reflexivity.
Qed.
...
Theorem length_0 {X : Type} : forall (l : list X),
	length l = 0 -> l = [].
Proof.
	induction l; simpl in *; intros.
	- reflexivity.
...
Theorem pal_des {X : Type} : forall (l : list X),
	 l = [] \/ (exists x, l = [x]) \/ 
	 (exists (x y : X) (l0 : list X), l = x :: l0 ++ [y]).
Proof.
	intros. induction l.
	- left. reflexivity.
	- destruct IHl.
		+ right. left. exists x. rewrite H. reflexivity.
		+ destruct H.
			* right. right. destruct H. exists x, x0, []. 
				simpl. rewrite H. reflexivity.
			* right. right. do 3 destruct H. rewrite H.
				exists x, x1, (x0 :: x2). simpl. reflexivity.
Qed.

Theorem pal_lim {X : Type} : forall  (n : nat) (l : list X),
	length l <= n -> l = rev l -> pal l.
Proof.
	induction n; simpl in *; intros.
	- inversion H. apply length_0 in H2. rewrite H2. 
		constructor.
	- destruct (pal_des l) as[H1|[[a Ha]|[a[b[s Hs]]]]]; subst;
	  try constructor; simpl in *.
	  rewrite rev_app_distr in H0.
	  simpl in H0. inversion H0. subst.
	  constructor. apply IHn. 
	  + apply Sn_le_Sm__n_le_m in H. rewrite app_length in H. 
	  		rewrite add_comm in H. simpl in H.
	  		apply le_trans with (S (length s)). 
	  		do 2 constructor. apply H.
	  	+ apply (f_equal _ _ rev) in H3. do 2 rewrite rev_app_distr in H3.
	  		simpl in H3. rewrite rev_involutive in H3. inversion H3.
	  		rewrite rev_involutive. assumption.
Qed.

Theorem pal_rev_2 {X : Type} : forall (l : list X), 
	 l = rev l -> pal l.
Proof.
	intros. apply (pal_lim (length l) l). constructor.
	assumption.
Qed.
...
Inductive disjoint {X : Type} : list X -> list X -> Prop :=
	| dis_e 						: disjoint [] []
	| dis_l x1 l1 l2 	: disjoint l1 l2 -> 
		~(In x1 l2) -> disjoint (x1 :: l1) l2
	| dis_r x2 l1 l2 	: disjoint l1 l2 -> 
		~(In x2 l1) -> disjoint l1 (x2 :: l2)
.
...
Inductive NoDup {X : Type} : list X -> Prop :=
	| dup_e 			: NoDup []
	| dup_1 l x	: NoDup l -> ~(In x l) -> NoDup (x :: l)
.

(** Finally, state and prove one or more interesting theorems relating
    [disjoint], [NoDup] and [++] (list append).  *)

Theorem NoDup_app {X : Type} : forall (l1 l2 : list X),
	...
	- split.
		+ inversion H. subst. constructor. 
			* specialize IHl1 with l2.
				apply IHl1 in H2. destruct H2. assumption.
			* unfold not in *. intros. apply H3.
				apply In_app_iff. left. assumption.
		+ inversion H. subst. specialize IHl1 with l2.
			apply IHl1 in H2. destruct H2. assumption.
Qed.

Theorem NoDup_sym_help {X : Type} : forall (l1 l2 : list X) (x : X),
...
Proof.
	...
	- constructor. apply IHl1.
		...
		+ inversion H.
			subst. contradict H0. unfold not in H0. 
			right. assumption.
		+	contradict H0. left.
			apply In_app_iff, or_commut, In_app_iff in H0.
...
Theorem NoDup_sym {X : Type} : forall (l1 l2 : list X),
	NoDup(l1 ++ l2) -> NoDup(l2 ++ l1).
Proof.
	induction l1; simpl in *; intros.
	- rewrite app_nil_r. assumption.
	- inversion H. apply NoDup_sym_help.
		+ apply IHl1. assumption.
		+ contradict H3. 
			apply In_app_iff, or_commut, In_app_iff in H3.
			assumption.
Qed.

Theorem disjoint_NoDup {X : Type} : forall (l1 l2 : list X),
	disjoint l1 l2 -> NoDup l1 -> NoDup l2 -> NoDup (l1 ++ l2).
Proof.
	intros * H. induction H; simpl in *; intros.
	- assumption.
	- inversion H1. subst. constructor. apply IHdisjoint.
		assumption. assumption. unfold not in *. intros. 
		apply In_app_iff in H3. destruct H3.
		+ apply H6. assumption.
		+ apply H0. assumption.
	- apply NoDup_sym_help.
		inversion H2. subst. apply IHdisjoint; assumption.
		contradict H0. apply In_app_iff in H0. destruct H0; trivial.
...
Proof.
	...
	- destruct H; subst.
		...
		+ apply IHl in H. do 2 destruct H.
			exists (x0 :: x1), x2. simpl. rewrite H.
			reflexivity.
Qed.
...
Inductive repeats {X:Type} : list X -> Prop :=
	| rep_s x l		: In x l -> repeats (x :: l)
	| rep_1 x l 		: repeats l -> repeats (x :: l)
.
...
Proof.
  ...
  - intros.	destruct (EM (In x l1')).
  		+ apply rep_s. assumption.
  		+ apply rep_1. simpl in *.
    		assert(H3 : In x l2).
  			{	apply (H x). simpl. left. reflexivity.	}
  			apply in_split in H3 as [s1[s2]]. apply (IHl1' (s1 ++ s2)).
  			* intros. specialize H with x0. subst.
    			assert (In x0 (s1 ++ x :: s2)).
    			{ apply (H (or_intror H3)). }
    			apply In_app_iff in H2. apply In_app_iff.
    			destruct H2. 
    			-- left. assumption.
    			-- right. simpl in *. destruct H2.
  			 		++ subst. contradiction.
  			  		++ assumption.
  			* subst. rewrite app_length in *. simpl in *.
  				rewrite add_comm in H0. simpl in H0.
  				unfold lt in *. apply Sn_le_Sm__n_le_m in H0.
...
Proof.
	...
	- intros. inversion H. subst.
		...
		+ left. split.
			* assumption.
			* simpl. assumption.
		+ right. exists s1, s2. inversion H1. subst. 
			repeat split; trivial.
	- intros. do 2 destruct H.
		+ apply (MApp [] re0 (a::s) re1 H H0).
		+ do 2 destruct H. destruct H0. rewrite H.
			apply (MApp (a::x) re0 x0 re1 H0 H1).
Qed.
(** [] *)
...
Proof.
	...
	- remember (Star re) as re'. 
		remember (a :: s) as l. 
		induction H; try inversion Heqre'; try inversion Heql.
		destruct s1; simpl in *.
		+ apply IHexp_match2 in Heqre'; subst.
			* destruct Heqre'. do 2 destruct H1. destruct H2.
				subst. exists x, x0. repeat split; trivial.
			* assumption.
		+ subst. inversion Heql. subst.
			exists s1, s2. repeat split; trivial.
	- do 3 destruct H. destruct H0. subst.
		apply (MStarApp (a :: x) x0 re H0 H1).
Qed.
(** [] *)
...
Lemma app_empty {T : Type}: forall l1 l2 : list T,
	l1 ++ l2 = [] -> l1 = [] /\ l2 = [].
Proof.
	intros.
	split.
...
Proof.
	...
	- apply ReflectF. unfold not. intros. inversion H.
	- inversion IHre1; inversion IHre2; simpl.
		+ apply ReflectT. apply (MApp [] re1 [] re2 H0 H2).
		+ apply ReflectF. unfold not in *. intros. inversion H3.
		  subst. apply app_empty in H4. destruct H4. subst.
		  	apply H2. assumption.
		+ apply ReflectF. unfold not in *. intros. inversion H3.
		  subst. apply app_empty in H4. destruct H4. subst.
		  	apply H0. assumption.
		+ apply ReflectF. unfold not in *. intros. inversion H3.
		  subst. apply app_empty in H4. destruct H4. subst.
		  	apply H0. assumption.
	- inversion IHre1; inversion IHre2; simpl.
		+ apply ReflectT. apply (MUnionL [] re1 re2 H0).
		+ apply ReflectT. apply (MUnionL [] re1 re2 H0).
		+ apply ReflectT. apply (MUnionR re1 [] re2 H2).
		+ apply ReflectF. unfold not in *. intros. inversion H3.
			* apply H0. assumption.
			* apply H2. assumption.
	- inversion IHre; simpl in *; apply ReflectT; constructor.
Qed.
(** [] *)
...
Proof.
	unfold derives.
	intros a re. generalize dependent a.	
	induction re; simpl in *; unfold is_der in *; intros.
	- split; intros; inversion H.
	- split; intros; inversion H.
	- split.
		+ intros. destruct ascii_dec.
			* inversion H. subst. constructor.
			* inversion H. subst. unfold not in *. 
				exfalso. apply n. reflexivity.
		+ intros. destruct ascii_dec in H.
			* inversion H. subst. constructor.
			* inversion H.
	- split.
		+ intros. unfold is_der in *. inversion H. subst.
			destruct (match_eps re1) eqn: H0.
			* destruct s1; simpl in *.
				-- apply MUnionL. apply IHre2.
					 simpl in *. rewrite <- H1. assumption.
				-- apply MUnionR. inversion H1. subst. constructor. 
					 apply IHre1. assumption.
					 assumption.
			* destruct s1; simpl in *.
				-- destruct (match_eps_refl re1). discriminate H0.
					 unfold not in *. apply H2 in H3. destruct H3.
				-- inversion H1. subst. constructor.
					 apply IHre1. assumption.
					 assumption.
		+ intros. unfold is_der in *.  
			destruct (match_eps re1) eqn: H0.
			* inversion H; subst.
				-- apply IHre2 in H3. destruct (match_eps_refl re1).
					 apply (MApp [] re1 (a :: s) re2 H1 H3).
					 discriminate H0.
				-- inversion H3. subst. apply IHre1 in H5.
					 apply (MApp (a :: s1) re1 s2 re2 H5 H6).
			* inversion H. subst. apply IHre1 in H4.
				apply (MApp (a :: s1) re1 s2 re2 H4 H5).
	- split; intros; unfold is_der in *.
		+ inversion H; subst.
		...
		+ inversion H; subst.
			...
			* apply MUnionR. apply IHre2. assumption.
	- split; intros; unfold is_der in *. 
		+ apply star_ne in H. do 3 destruct H. destruct H0.
			subst. constructor.
			-- apply IHre. assumption.
			-- assumption.
		+ inversion H. subst.
			apply IHre in H3.
			apply (MStarApp (a::s1) s2 re); trivial.
Qed.
(** [] *)
...
Proof.
	...
	- intros. destruct (match_eps_refl re).
		+ apply ReflectT. assumption.
		+ apply ReflectF. assumption.
	- intros. destruct (derive_corr x re s).
		destruct (IHs (derive x re)).
		+ apply ReflectT. apply H0. assumption.
		+ apply ReflectF. contradict H1. apply H. 
			assumption.
Qed.
(** [] *)
