...
Proof.
	...
	- induction m. reflexivity. rewrite add_comm in H. discriminate.
Qed.
(** [] *)
...
Theorem or_commut : forall P Q : Prop,
  P \/ Q  -> Q \/ P.
Proof.
	intros. destruct H. right. apply H. left. apply H.
Qed.
(** [] *)
...
Fact not_implies_our_not : forall (P:Prop),
  ~ P -> (forall (Q:Prop), P -> Q).
Proof.
	intros A B C D. destruct B. apply D.   
Qed.
(** [] *)
...
Theorem contrapositive : forall (P Q : Prop),
  (P -> Q) -> (~Q -> ~P).
Proof.
	intros A B C. unfold not. intros. apply H. apply C. apply H0. 
Qed.
(** [] *)
...
Theorem not_both_true_and_false : forall P : Prop,
  ~ (P /\ ~P).
Proof.
	intros. unfold not. intros. destruct H. apply H0. apply H. 
Qed.
(** [] *)
...
Theorem iff_trans : forall P Q R : Prop,
  (P <-> Q) -> (Q <-> R) -> (P <-> R).
Proof.
	intros. split. intros. apply H0. apply H. apply H1.
	intros. apply iff_sym in H. apply H. 
	apply iff_sym in H0. apply H0. apply H1. 
Qed.
(** [] *)
...
Proof.
	...
	- intros. destruct H.
		...
		+ split. right. apply H. right. apply H.
...
Proof.
	intros.
	unfold not.
	intros. destruct H0 as [a H0]. apply H0.
	apply (H a).
Qed.
(** [] *)
  ...
  - intros.
    induction l; simpl in *.
  		...
  		+ destruct H; subst.
  		  ...
  		  apply IHl in H.
  		  destruct H. exists x0. destruct H. split. apply H.
  		  right. apply H0.
  - intros.
    destruct H as [a [H1 H2]]; subst.
...
Proof.
	...
	- intros. 
	  induction l; simpl in *; destruct H0; subst; destruct H.
		+ apply H.
		+ apply (IHl H1 H0).
Qed.
(** [] *)
...
Proof.
	intros. unfold combine_odd_even. destruct odd.
	- apply H. reflexivity.
...
Theorem combine_odd_even_elim_odd :
  forall (Podd Peven : nat -> Prop) (n : nat),
    ...
    Podd n.
Proof.
	intros. unfold combine_odd_even in *. destruct odd.
	- apply H.
	- discriminate H0.
Qed.
...
Compute rev_append [1; 2; 3] [10 ; 20; 30].

Theorem tr_rev_app : forall X, forall l1 l2 : list X, 
...
Proof.
	...
	induction l1.
	- simpl. reflexivity.
	- simpl. intros. rewrite IHl1. rewrite (IHl1 [x]).
...
Proof.
	...
	induction x.
	- simpl. reflexivity.
	- simpl. rewrite tr_rev_app. rewrite IHx. reflexivity.
Qed.
(** [] *)
...
Lemma double_x1 : forall x, S (S (double x)) = double (x + 1).
Proof.
	intros. induction x.
	- simpl. reflexivity.
	- simpl. do 2 f_equal. apply IHx.
Qed.

(** **** Exercise: 3 stars, st	andard (even_double_conv) *)
...
Proof.
  ...
  - rewrite even_S. destruct even. 
  		+ destruct IHn. exists x. rewrite H. simpl. reflexivity.
  		+ destruct IHn. exists (x + 1). simpl. rewrite H.
  			apply double_x1.
Qed.
...
Proof.
	intros. split. 
	- intros. destruct b1, b2; split; try reflexivity; try discriminate H.
	-	intros. destruct b1, b2; simpl in *; destruct H. reflexivity. 
		+ apply H0.
		+ apply H.
		+ apply H.
Qed.
...
Proof.
	...
	- intros. destruct H.
		...
		+ rewrite H. destruct b1; simpl; reflexivity. 
Qed.
(** [] *)
...
Proof.
	...
	- unfold not. intros. rewrite H0 in H.
		rewrite eqb_true in H. discriminate H.
	- intros. unfold not in H. rewrite <- not_true_iff_false.
		unfold not. intros. apply H. apply eqb_eq. apply H0.
Qed.
...
Fixpoint eqb_list {A : Type} (eqb : A -> A -> bool)
	...
	match l1, l2 with
	| [], [] => true
	| x_1 :: l_1, x_2 :: l_2 => (eqb x_1 x_2) && (eqb_list eqb l_1 l_2)
	|	_, _ => false
	end.
...
Proof.
    ...
    - induction l1, l2; intros.
			...
			+ discriminate H0.
			+ discriminate H0.
			+ simpl in *. destruct (eqb x x0) eqn: H1.
				* simpl in *. apply H in H1. 
					apply IHl1 in H0. subst. reflexivity.
				* simpl in *. discriminate H0.
		- intros. subst. induction l2; simpl; try reflexivity.
			rewrite IHl2. destruct (eqb x x) eqn:H1 ; simpl. reflexivity.
			rewrite <- H1. apply H. reflexivity.
Qed.
...
Proof.
	...
	- simpl in *. split.
		+ intros. split.
			* destruct (test x) eqn: H0. reflexivity. exfalso.
			  discriminate H.
			* destruct (test x) eqn: H0; simpl in *.
				apply IHl. apply H. discriminate H. 
		+ intros [H1 H2]. destruct test; simpl in *.
			apply IHl. apply H2. apply H1.
Qed.
...
Proof.
	unfold excluded_middle, not in *. intros.
	specialize H with (P x).
	destruct H. apply H. exfalso. apply H0. exists x. apply H.
Qed.
(** [] *)
...
Proof.
	unfold de_morgan_not_and_not, double_negation_elimination, not.
	intros. apply H. intros. apply H1. left.
	apply H. intros. apply H1. right. apply H. intros.
	apply H0. split.
	- apply H2.
	- apply H3.
Qed.
...
Proof.
	...
	intros.
	apply H. intros. destruct H1. apply H1. intros.
...
Proof.
	unfold peirce, implies_to_or, not.
	intros.
	destruct (H P P).
	- intros p. apply p.
	- apply H0. intros p. contradiction. 
	- apply H1.
Qed.
(* 2021-08-11 15:08 *)
